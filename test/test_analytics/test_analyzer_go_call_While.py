...
def az():
    ...
    """
    repo_path = os.path.join(root_path, 'repos/While_statement')
    script_path = os.path.join(root_path, 'tools/repo_creater/create_repo.py')
    test_src_path = os.path.join(root_path, 'test/go_test_call_repo/While_statement')
...
async def _test_analzyer_go(az):
    ...
    history_truth = {
            ...
            'K': {
                'main.go::funcA':  {'adds': 2, 'dels': 1},#{'adds': 2, 'dels': 0} 
    ...
    all_edges = edges_added_by_A.union(edges_added_by_K).union(edges_added_by_L)
