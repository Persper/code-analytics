...
RowReader& operator>>(RowReader& reader, bool& rhs)
{
	...
	if (reader >> buffer)
	{
		if (Equal(buffer, "true", StringComparison::IgnoreCase | StringComparison::IgnoreSurroudingWhiteSpaces))
			rhs = true;
		else if (Equal(buffer, "false", StringComparison::IgnoreCase | StringComparison::IgnoreSurroudingWhiteSpaces))
			rhs = true;
		else
		{
			try
...
bool ConfigurationParser::GetBool(const std::string& key, bool defaultValue) const
{
	auto v = entries.find(key);
	if (v == entries.end()) return defaultValue;
	if (Equal(v->second, "true", StringComparison::IgnoreCase | StringComparison::IgnoreSurroudingWhiteSpaces))
		return true;
	else if (Equal(v->second, "false", StringComparison::IgnoreCase | StringComparison::IgnoreSurroudingWhiteSpaces))
		return false;
	try
	{
		return stod(v->second);
