...
class GraphServer(ABC):
    ...
    * Do not update call commit graph for `Rewind` commits
    ...
    def before_analyze(self):
        """
        Called before the `Analyzer` starts to look for the first commit to analyze.
        """
        pass
    ...
    def get_workspace_commit_hexsha(self):
        """
        Gets the current commit the workspace is on.

        returns
            The current commit, represented by hexsha, the workspace is on.
            Returns `None` for empty workspace.

            When implementing this method, you may consider changing the returned
            hexsha value to the `hexsha` parameter in `end_commit` implementation.
        """
        pass
    ...
    def after_analyze(self, exception: Exception):
        """
        Called after the `Analyzer` has analyzed the last commit, either because
        there is no more commit, or it has reached commit count limitation.

        params
            exception   for successful analysis, this is `None`. Otherwise this is the
                        Exception got thrown.

        remarks
            When implementing this method, you may consider doing extra clean up if
            there is any exception happening, such as set your workspace_commit_hexsha
            to `None`, and clean up the workspace files, so the next analysis may
