...
class Analyzer:
    ...
    def compute_project_complexity(self, commit_black_list: Optional[Set] = None):
        """Evaluates project complexity (single language)."""
        return self.graph.eval_project_complexity(commit_black_list=commit_black_list)
    ...
    def register_commit_analyzers(self, analyzer_params: Tuple):
        """Register commit analyzing fun, executed when analyzing commits
        analyze_fun:
            input: Analyzer, Commit, Diffs, *params[1:]
            output: None
        No returns
        """
        self._commit_analyzer_funs.append(analyzer_params)

    def get_commit_analyzer_data(self):
        """_commit_analyzer_data stores data generated by _commit_analyzer_funs
        """
        return self._commit_analyzer_data
            ...
            for commit in self._repo.iter_commits(commitSpec,
                ...
                if len(commit.parents) > 1 and not self._firstParentOnly:
                    ...
                    if lastCommit != expectedParentCommit:
                        printCommitStatus(logging.INFO, "Rewind to parent: {0}.".format(
                            expectedParentCommit or "<empty>"))
    ...
    async def _analyzeCommit(self, commit: Union[Commit, str], parentCommit: Union[Commit, str],
        ...
        # t0: Total time usage
        t0 = time.monotonic()
        self._observer.onBeforeCommit(
            self._lang, self._analyzing_index)
        ...
        if diff_index:
            for diff in diff_index:
                ...
                if old_src or new_src:
                    result = self._graphServer.update_graph(old_fname, old_src, new_fname, new_src, diff.diff)
                    if asyncio.iscoroutine(result):
                        await result
        ...
        self._observer.onAfterCommit(self._lang, self._analyzing_index)
    ...
    def _filter_monolithic_commit(self, commit: Commit, seeking_mode: CommitSeekingMode) -> CommitSeekingMode:
        ...
        if seeking_mode == CommitSeekingMode.NormalForward and len(commit.parents) <= 1:
            ...
            print('_filter_monolithic_commit commit:',
                  commit.hexsha, 'changed_lines:', changed_lines)
...
class AnalyzerObserver(ABC):
    ...
    def onBeforeCommit(self, language: str, index: int):
        """
        Called after the observed Analyzer has finished analyzing a commit.
        Params:
            language: the  language attribute of analyzer.
    ...
    def onAfterCommit(self, language: str, index: int):
        """
        Called after the observed Analyzer has finished analyzing a commit.
        Params:
            language: the  language attribute of analyzer.
